...
export interface PromptInput {
   ...
   */
  protocols?: { orchestrator?: string; static?: string; functional?: string };
  /**
...
export function buildPrompt(input: PromptInput): { app_name: string; depth: 'static' | 'full'; prompt: string } {
  const f = input;
  const depth = f.depth || 'full';
  ...
  const protocolsInline = Boolean(f.protocols && (f.protocols.orchestrator || f.protocols.static));
  const cb = f.callback;
  ...
  L.push('');
  L.push('Parameters: repo=' + repo + ' ; app_name=' + app + ' ; depth=' + depth + '.');
  L.push('');
  if (cb) {
  ...
  L.push(protocolsInline
    ? 'TOOL ACCESS (critical): you need the repository and, for a full run, a browser. There is NO wiki in this installation - do not call docmost-mcp or any page-fetching tool, and do not treat a missing wiki as a reason to error. The protocol is given to you in full below.'
  ...
  if (full) {
    ...
    L.push('- report_markdown: the full report body following the orchestrator report template (omit PR-only lines); mark the Functionality section as not run (depth=static).');
  }
  ...
  if (protocolsInline) {
    ...
    if (P.static) L.push(NL + '--- STATIC REVIEW PROTOCOL (leaf) ---' + NL + P.static);
    if (full && P.functional) L.push(NL + '--- FUNCTIONAL REVIEW PROTOCOL (leaf) ---' + NL + P.functional);
  }
  const prompt = L.join(NL);
    return { app_name: app, depth: depth as 'static' | 'full', prompt };
  }

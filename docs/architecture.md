...
| B13 | **Re-eligible when an operator asks** | — (n8n's only manual verb is a forced run, which dispatches immediately) | ➕ a subject flagged since its last attempt is eligible regardless of `FRESH_DAYS`. The third of these and the only one that is not about the world changing: it is the answer to an app kept out of the backlog by a *sibling* section's completion while the section that matters keeps blocking, which B10 and B11 cannot see and no automatic rule may fix — one that made `blocked` eligible would re-pick that app every cooldown until the bench came back. A timestamp, spent by the next attempt, so one flag buys one look. The tick says `· flagged for re-audit` |
